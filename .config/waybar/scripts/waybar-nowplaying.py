...
parser.add_argument(
    '--playing-only',
    action='store_true',
    help="if set, don't show any output when the current song is pause",
    dest='playing_only',
)
...
def push_output(text, player):
    logger.info('Writing output')

    output = {'text': text,
              'class': 'custom-nowplaying',
              'alt': player}
...
try:
    ...
    media_interfaces = ["cider2", "cider", "spotify"]
    bus = None
